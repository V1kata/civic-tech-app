...
# Convert from Web Mercator (EPSG:3857) -> WGS84 (EPSG:4326)
...
class WaterDisruptionScraper:
    ...
    def to_wgs84(self, x: float, y: float) -> tuple[float, float]:
        """Convert from EPSG:3857 (Web Mercator) to WGS84 (latitude, longitude)."""
    ...
    def make_id(self, alert_id: str, location: str) -> str:
        """Generate a stable unique ID based on alert ID only."""
        slug = str(location).lower()[:20].replace(" ", "_").replace(".", "")
        return f"water_{slug}_{alert_id}"
    ...
    def fetch_water_stops(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.session.get(
                ...
                params={
                    "f": "json",
                    "where": "ACTIVESTATUS = 'In Progress'",  # само активни спирания
                    "outFields": "*",
                    "returnGeometry": "true",
                    "outSR": "102100",                         # Web Mercator за координатите
            ...
            if rings:
                try:
                    ...
                    for ring in rings:
                        wgs84_ring = []
                        for pt in ring:
                            r_lat, r_lon = self.to_wgs84(pt[0], pt[1])
                            wgs84_ring.append([r_lat, r_lon])
                        polygon_wgs84.append(wgs84_ring)
                except Exception:
                    pass
